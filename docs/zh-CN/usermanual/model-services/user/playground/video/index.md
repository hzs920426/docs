...
| 管理对象 | 视频模型、提示词、参考素材、生成参数和任务结果 |

#### 新手理解

视频生成像模型试用台。先选择可用的视频模型和供应方，再填写输入并调整参数，提交后根据结果或错误提示决定是否继续接入。
...
第一次体验时，建议先选择视频模型，再核对供应方和 Personal Key，随后配置输入与参数，最后提交并查看结果。每次只调整少量参数，便于比较差异。

#### 小白先看

| 场景 | 先做 | 不要直接做 |
| --- | --- | --- |
| 不知道选哪个视频模型 | 先打开模型选择器比较状态和能力 | 直接提交默认模型 |
...
点击生成按钮会产生真实模型调用，可能消耗额度并创建调用日志、账务记录或异步任务。提交前应确认模型、参数、素材授权、预计费用和排队时间。
:::

## 页面说明

页面包含视频模型选择、输入区、参数区、Personal Key 选择和结果区。提交操作会产生真实调用，并可能形成用量或计费记录。

页面截图：

![视频生成页面](./images/video-list-public.png)
...
![选择视频模型](./images/manual-select-video-model-public.png)

上图展示模型选择器。重点比较供应方实例的能力、价格、性能和可用状态。

![视频模型选择参考](./images/select-model-public.png)
...
2. 填写视频描述并按页面要求上传参考素材，设置时长、画面比例和生成参数；确认素材授权和费用后，点击 **"生成"**。
3. 提交后查看结果、耗时、用量和错误提示；失败时先核对模型状态、额度、参数和限流情况。
4. 记录问题时只保留脱敏后的请求标识、模型名称和发生时间，不复制真实密钥或完整敏感输入。

![配置视频生成](./images/manual-configure-video-public.png)
...
- 视频生成可能创建异步任务或排队记录，重复提交前应先确认现有任务状态。
...
| 结果区可查看 | 页面显示生成结果、任务进度、错误提示或空状态。 | 如无生成记录，确认输入区和参数区仍然可见。 |
